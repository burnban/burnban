...
// cmdUsage prices local agent traffic that may never touch the proxy.
...
func cmdUsage(args []string) error {
	...
	geminiDir := fs.String("gemini-dir", localusage.DefaultGeminiDir(home), "Gemini CLI project chat logs")
	copilotDir := fs.String("copilot-dir", localusage.DefaultCopilotDir(home), "GitHub Copilot CLI session event logs")
	cursorDB := fs.String("cursor-db", localusage.DefaultCursorDB(home), "Cursor global composer metadata database")
	openCodeDB := fs.String("opencode-db", localusage.DefaultOpenCodeDB(home), "OpenCode usage database")
	...
	gooseDB := fs.String("goose-db", localusage.DefaultGooseDB(home), "Goose session database")
	planCost := fs.Float64("plan-cost", 0, "what you pay per month across plans, used to calculate the plan multiple")
	...
	if !*noAutoMetered {
		metered = append(metered, localusage.DetectMeteredProviders(home)...)
	}
	until := time.Now()
	...
	shareCard := localusage.NewShareCard(report, label, *planCost)
	if *asJSON {
	...
	if !report.HasUsage {
		...
		for _, provider := range report.Providers {
			fmt.Printf("  %-12s %s\n", usageSourceTitle(provider.Provider), terminalText(provider.Dir, 240))
			if provider.Detail != "" {
	...
	if *share {
		fmt.Print(renderUsageShareCard(shareCard, stdoutIsTerminal() && os.Getenv("NO_COLOR") == ""))
		return nil
	}
	...
	for _, provider := range report.Providers {
		...
		fmt.Printf("\n%s  %s · %d sessions · %s\n", usageSourceTitle(provider.Provider), terminalText(provider.Dir, 240), provider.Sessions, tag)
		if provider.Detail != "" {
			fmt.Printf("  scan issue: %s\n", terminalText(provider.Detail, 240))
	...
	if report.MeteredUSD > 0 {
		fmt.Printf("REAL API SPEND $%.2f already billed per token\n", report.MeteredUSD)
	}
	...
	fmt.Println()
	if *planCost > 0 {
		fmt.Printf("\n  subscription usage at API rates vs your $%.0f/mo plan → %.1fx the plan price\n", *planCost, subPace/(*planCost))
	} else {
		...
		// it), so the plausible plan multiple is about the same on every tier.
		...
		const plausiblePlanMultiple = 30.0
		type plan struct {
			name string
			usd  float64
		}
		...
		for _, provider := range report.Providers {
			...
			// separately billed events that must not inflate the plan multiple.
			...
			for _, item := range providerPlans {
				if pace/item.usd <= plausiblePlanMultiple {
...
func renderUsageShareCard(card localusage.ShareCard, color bool) string {
	...
	header := "BURNBAN LOCAL USAGE · " + strings.ToUpper(card.Window)
	if card.Partial {
...
func defaultHermesDB(home string) string {
	return localusage.DefaultHermesDB(home)
}

func defaultOpenClawDir(home string) string {
	return localusage.DefaultOpenClawDir(home)
}

func usageSourceTitle(provider string) string {
	switch provider {
	case "claude-code":
		return "CLAUDE CODE"
	case "codex":

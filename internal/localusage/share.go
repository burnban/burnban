...
// ShareCard is the structured form of the compact local-usage output.
...
func NewShareCard(report Report, window string, planCostUSD float64) ShareCard {
	...
	// Only flat-rate subscription usage belongs in the plan comparison. Real
	// pay-per-token spend (BYO-key agents, API-key auth) is already billed and
	// must not inflate the multiple.
	planEquivalentUSD := report.PlanEquivalentUSD()
	monthlyPaceUSD := math.Round(planEquivalentUSD/windowDays*30*100) / 100
	multiplier := math.Round(monthlyPaceUSD/planCostUSD*10) / 10
	return ShareCard{
		HasUsage:         report.HasUsage,
		APIEquivalentUSD: planEquivalentUSD,

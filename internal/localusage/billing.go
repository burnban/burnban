...
// rather than part of the flat-rate plan comparison. It reflects current auth
// state, not the billing mode of each historical session (the logs never
// record that), so a manual override still exists for overage and mixed windows.

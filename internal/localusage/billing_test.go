...
func TestReportPlanEquivalentUSD(t *testing.T) {
	// Unclassified report (both buckets zero) falls back to the full total.
	if got := (Report{Totals: Totals{APIUSD: 100}}).PlanEquivalentUSD(); got != 100 {
	...
	if got := r.PlanEquivalentUSD(); got != 30 {
...
func TestNewShareCardUsesSubscriptionOnly(t *testing.T) {
	report := Report{
		...
		MeteredUSD:      10000, // real spend must not inflate the plan multiple
